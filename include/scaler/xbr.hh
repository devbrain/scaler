...
static uint32_t dist(uvec3 A, uvec3 B) {
    ivec3 A_yuv = rgbToYuv(A);
    ivec3 B_yuv = rgbToYuv(B);
    uvec3 diff = abs(A_yuv - B_yuv);
    return (diff.x * Y_COEFF) + (diff.y * U_COEFF) + (diff.z * V_COEFF);
}

template<typename T>
SDL_Surface* scaleXbr(const input_image<T>& src) {
    ...
    for (int y = 0; y < src.height(); y++) {
        for (int x = 0; x < src.width(); x++) {
            ...
            if (edr_bot_right) {
                ...
                if (F == G && H == C) {
                    three   = mix(three, new_color, 0.75f);
                    two     = mix(two, new_color, 0.25f);
                    one     = mix(one, new_color, 0.25f);
                } else if (F == G) {
                    three   = mix(three, new_color, 0.75f);
                    two     = mix(two, new_color, 0.25f);
                } else if (H == C) {
                    three   = mix(three, new_color, 0.75f);
                    one     = mix(one, new_color, 0.25f);
                } else { three = mix(three, new_color, 0.5f); }
            }
            if (edr_bot_left) {
                T new_color = (dist(E, H) <= dist(E, D)) ? H : D;
                if (A == H && D == I) {
                    two     = mix(two, new_color, 0.75f);
                    zero    = mix(zero, new_color, 0.25f);
                    three   = mix(three, new_color, 0.25f);
                } else if (A == H) {
                    two     = mix(two, new_color, 0.75f);
                    zero    = mix(zero, new_color, 0.25f);
                } else if (D == I) {
                    two     = mix(two, new_color, 0.75f);
                    three   = mix(three, new_color, 0.25f);
                } else { two = mix(two, new_color, 0.5f); }
            }
            if (edr_top_left) {
                T new_color = (dist(E, D) <= dist(E, B)) ? D : B;
                if (D == C && B == G) {
                    zero    = mix(zero, new_color, 0.75f);
                    one     = mix(one, new_color, 0.25f);
                    two     = mix(two, new_color, 0.25f);
                } else if (D == C) {
                    zero    = mix(zero, new_color, 0.75f);
                    ...
                    zero    = mix(zero, new_color, 0.75f);
                    two     = mix(two, new_color, 0.25f);
                } else { zero = mix(zero, new_color, 0.5f); }
            }
            if (edr_top_right) {
                T new_color = (dist(E, B) <= dist(E, F)) ? B : F;
                if (B == I && F == A) {
                    one     = mix(one, new_color, 0.75f);
                    three   = mix(three, new_color, 0.25f);
                    zero    = mix(zero, new_color, 0.25f);
                } else if (B == I) {
                    one     = mix(one, new_color, 0.75f);
                    ...
                    one     = mix(one, new_color, 0.75f);
                    zero    = mix(zero, new_color, 0.25f);
                } else { one = mix(one, new_color, 0.5f); }
            }

            // Final assignments
            int dst_x = 2 * x;
            int dst_y = 2 * y;
            result.set_pixel(dst_x, dst_y, zero);
            result.set_pixel(dst_x + 1, dst_y, one);
            result.set_pixel(dst_x, dst_y + 1, two);
            result.set_pixel(dst_x + 1, dst_y + 1, three);
        }
    }
