...
SCALER_FORCE_INLINE ColorDiff rgbToHqColorspaceFP(const vec3<unsigned int>& rgb) {
    ...
    return ColorDiff{
        (r + g + b) << 6,  // (r + g + b) / 4 in FP
        (r - b) << 6,      // (r - b) / 4 in FP
        ((-r + 2*g - b) << 5) // (-r + 2g - b) / 8 in FP
    };
}
...
struct PatternCache {
    ...
    SCALER_FORCE_INLINE void buildAllPatterns(const PixelType& w0, const PixelType& w1, const PixelType& w2,
        ...
        patterns[0] = 0;
        patterns[0] |= isDifferent(w0, w4) ? 0x01 : 0;
        patterns[0] |= isDifferent(w1, w4) ? 0x02 : 0;
        patterns[0] |= isDifferent(w2, w4) ? 0x04 : 0;
        patterns[0] |= isDifferent(w3, w4) ? 0x08 : 0;
        patterns[0] |= isDifferent(w5, w4) ? 0x10 : 0;
        patterns[0] |= isDifferent(w6, w4) ? 0x20 : 0;
        patterns[0] |= isDifferent(w7, w4) ? 0x40 : 0;
        patterns[0] |= isDifferent(w8, w4) ? 0x80 : 0;
        
        // FlipX pattern (w0↔w2, w3↔w5, w6↔w8)
        patterns[1] = 0;
        patterns[1] |= isDifferent(w2, w4) ? 0x01 : 0;  // w0 position
        patterns[1] |= isDifferent(w1, w4) ? 0x02 : 0;  // w1 stays
        patterns[1] |= isDifferent(w0, w4) ? 0x04 : 0;  // w2 position
        patterns[1] |= isDifferent(w5, w4) ? 0x08 : 0;  // w3 position
        patterns[1] |= isDifferent(w3, w4) ? 0x10 : 0;  // w5 position
        patterns[1] |= isDifferent(w8, w4) ? 0x20 : 0;  // w6 position
        patterns[1] |= isDifferent(w7, w4) ? 0x40 : 0;  // w7 stays
        patterns[1] |= isDifferent(w6, w4) ? 0x80 : 0;  // w8 position
        
        // FlipY pattern (w0↔w6, w1↔w7, w2↔w8)
        patterns[2] = 0;
        patterns[2] |= isDifferent(w6, w4) ? 0x01 : 0;  // w0 position
        patterns[2] |= isDifferent(w7, w4) ? 0x02 : 0;  // w1 position
        patterns[2] |= isDifferent(w8, w4) ? 0x04 : 0;  // w2 position
        patterns[2] |= isDifferent(w3, w4) ? 0x08 : 0;  // w3 stays
        patterns[2] |= isDifferent(w5, w4) ? 0x10 : 0;  // w5 stays
        patterns[2] |= isDifferent(w0, w4) ? 0x20 : 0;  // w6 position
        patterns[2] |= isDifferent(w1, w4) ? 0x40 : 0;  // w7 position
        patterns[2] |= isDifferent(w2, w4) ? 0x80 : 0;  // w8 position
        
        // FlipXY pattern (both flips)
        patterns[3] = 0;
        patterns[3] |= isDifferent(w8, w4) ? 0x01 : 0;  // w0 position
        patterns[3] |= isDifferent(w7, w4) ? 0x02 : 0;  // w1 position
        patterns[3] |= isDifferent(w6, w4) ? 0x04 : 0;  // w2 position
        patterns[3] |= isDifferent(w5, w4) ? 0x08 : 0;  // w3 position
        patterns[3] |= isDifferent(w3, w4) ? 0x10 : 0;  // w5 position
        patterns[3] |= isDifferent(w2, w4) ? 0x20 : 0;  // w6 position
        patterns[3] |= isDifferent(w1, w4) ? 0x40 : 0;  // w7 position
        patterns[3] |= isDifferent(w0, w4) ? 0x80 : 0;  // w8 position
    }
};
...
struct OmniScaleCore {
    ...
    SCALER_FORCE_INLINE void setPattern(int p) {
        pattern = p;
    }
    ...
    inline PixelType interpolateCorner(float px, float py) const noexcept {
        ...
        if ((p_low == 0x03 && p_high == 0x10) ||  // 0x1B,0x03
            ...
            (p_low == 0x43 && p_high == 0x60)) {  // 0x6B,0x43
            return mix(w4, w3, weight_x);
        }
        ...
        if ((p_low == 0x09 && p_high == 0x40) ||  // 0x4B,0x09
            ...
            (p_low == 0x19 && p_high == 0x30)) {  // 0x3B,0x19
            return mix(w4, w1, weight_y);
        }
        
        // Diagonal patterns - use lookup for isDifferent checks
        if (p_low == 0x37 || p_low == 0x13) {
            if (p_high == 0xB0 || p_high == 0xD0) {  // 0xBF,0x37 or 0xDB,0x13
        ...
        if (p_low == 0x49 || p_low == 0x6D) {
            if (p_high == 0xD0 || p_high == 0xE0) {  // 0xDB,0x49 or 0xEF,0x6D
...
SCALER_HOT
auto scaleOmniScale2x(const InputImage& src, int scale_factor = 2) 
    -> OutputImage {
...
template<typename InputImage, typename OutputImage>
SCALER_HOT
auto scaleOmniScale3x(const InputImage& src, int scale_factor = 3) 
    -> OutputImage {
