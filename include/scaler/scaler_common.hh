...
namespace scaler {
    ...
    */
    ...
    inline static uvec3 rgbToYuv(const uvec3& val) noexcept {
        ...
        unsigned int y = static_cast<unsigned int>((Y_R * r + Y_G * g + Y_B * b) >> 16);
        unsigned int u = static_cast<unsigned int>(((U_R * r + U_G * g + U_B * b) >> 16) + 128);
        unsigned int v = static_cast<unsigned int>(((V_R * r + V_G * g + V_B * b) >> 16) + 128);
    ...
    [[maybe_unused]] inline static uint32_t rgbToYuv(uint32_t val) noexcept {
        ...
        int32_t r = static_cast<int32_t>((val & 0xFF0000) >> 16);
        int32_t g = static_cast<int32_t>((val & 0x00FF00) >> 8);
        int32_t b = static_cast<int32_t>(val & 0x0000FF);

        uint32_t y = static_cast<uint32_t>((Y_R * r + Y_G * g + Y_B * b) >> 16);
        uint32_t u = static_cast<uint32_t>(((U_R * r + U_G * g + U_B * b) >> 16) + 128);
        uint32_t v = static_cast<uint32_t>(((V_R * r + V_G * g + V_B * b) >> 16) + 128);
        return (y << 16) | (u << 8) | v;
    }
}
